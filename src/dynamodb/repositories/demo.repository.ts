...
import { Record } from 'src/demo/models/model';
...
export class DemoRepository extends BaseRepository<Record> {
  ...
  createContact(contact: Contact): Promise<any> {
    return this.create(contact,UpdateEntity.Customer)
  }
  ...
  updateDocument(document: Partial<Document>,id:string,key?:string): Promise<any> {
    return this.updateMap(document,UpdateEntity.Document,id,key)
  }

  updateExtractions(extractions: Partial<Extractions>,id:string,key?:string): Promise<any> {
